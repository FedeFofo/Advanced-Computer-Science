public interface ElementalAttacks {
    ...
    default void attack(Monster opponent) {
        ...
        int chance = Utils.generateRandomNumber(0, 100);
    ...
    default void performSingleElementalAttack(int attackNumber, Monster opponent) {
        ...
        if (attackNumber == 1) {
            ...
            attackElementType = thisMonster.attackElementOne;
        } else {
            attackName = thisMonster.attackNameTwo;
            attackElementType = thisMonster.attackElementTwo;
        }

        int baseDamage = 10;

        double stab;
        if (attackElementType == thisMonster.element) {
            stab = 1.5;
        } else {
            stab = 1.0;
        }

        double effectivenessMultiplier = getElementalMultiplier(attackElementType, opponent.getElement());

        boolean crit = didAttackCrit(thisMonster.speed);
        ...
        int totalDamage = (int) (baseDamage * ((double) thisMonster.attack / opponent.getDefense()) * stab
                * effectivenessMultiplier * critMultiplier);

        opponent.takeDamage(totalDamage);

        System.out.print(thisMonster.name + " used " + attackName + "! ");
        ...
        if (crit) {
            System.out.print("It was a critical hit! ");
        }
        System.out.println();
    }
    ...
    default void performDoubleDownAttack(Monster opponent) {
        ...
        if (thisMonster.hasUsedDoubleDown) {
            System.out.println(
                    thisMonster.name + " tried to use their double down attack, but it has already been used!");
            return;
        }
